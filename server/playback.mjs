...
export function createPlaybackService(deps) {
  ...
  const timers = setInterval(() => {
    ...
    for (const session of sessions.values()) {
      ...
      // so it is released immediately instead of waiting out the whole lease. A
      // session that never received a heartbeat is reclaimed on the shorter
      // pre-heartbeat window, because its client is gone rather than idle.
      if (sourceSignature(session.media) !== session.metadata.sourceSignature) { release(session); continue; }
      const window = session.heartbeatSeen ? settings.leaseSeconds : settings.initialLeaseSeconds;
      if (now - session.lastSeen > window * 1000) release(session);
    }
  ...
  async function handleRequest(request, response, url, pathname) {
    ...
    try {
      ...
      if (mediaRoute) {
        ...
        // or switching media) must release it, otherwise it lingers for the whole
        // lease with heartbeat traffic from a page that no longer exists.
        let handedOff = false;
        ...
        } catch (error) { handedOff = false; abandon(); throw error; }
        return true;
      }
      ...
      if (!resource && request.method === "PATCH") {
        ...
        if (body.generation !== undefined && body.generation !== session.generation) throw playbackError("STALE_SESSION", "播放请求已被替换", 409);
      ...
      // Fetching fragments proves the client is alive and playing, so it keeps
      // the lease fresh even if a heartbeat is delayed by network timing.
