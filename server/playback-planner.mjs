...
export const PLAYBACK_DEFAULTS = Object.freeze({
  ...
  leaseSeconds: 45, initialLeaseSeconds: 20, releaseGraceSeconds: 5, noOutputSeconds: 30,
...
export function normalizePlaybackSettings(raw = {}) {
  ...
  const ranges = { cacheMaxBytes: [64 * 1024 ** 2, 1024 ** 4], cacheTtlSeconds: [60, 604800],
    ...
    leaseSeconds: [15, 180], initialLeaseSeconds: [10, 180], releaseGraceSeconds: [0, 30], noOutputSeconds: [5, 120],
  ...
  // Browsers routinely cancel the release request while a page is navigating, so
  // a session that never received a single heartbeat is reclaimed on this shorter
  // window instead of occupying a stream slot for the whole lease. Stored settings
  // from before this option keep working: the window is clamped to the heartbeat
  // rule rather than rejected.
  result.initialLeaseSeconds = Math.min(result.leaseSeconds, Math.max(result.initialLeaseSeconds, result.heartbeatSeconds * 2));
  return result;
}
