...
export async function api<T>(url: string, body?: unknown, method = body === undefined ? "GET" : "POST", signal?: AbortSignal): Promise<T> {
  ...
  if (!response.ok) throw Object.assign(new Error(data.error || `请求失败（${response.status}）`), { code: data.code, status: response.status });
  return data;
}
...
export class PlaybackCore extends EventTarget {
  ...
  async seek(time: number) {
    ...
    if (local && !this.switching) { this.video.currentTime = Math.max(0, target - this.state.timeOffset); this.emit("seeking", { currentTime: target, seeking: true }); void this.keepAlive(); }
