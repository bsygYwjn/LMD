...

{
  ...
  const tkhd = boxes(trakBox).find(item => item.type === "tkhd").data;
  assert.equal(tkhd.readUInt32BE(20), 42_500, "tkhd 时长使用影片时间刻度");
  assert.equal(tkhd.readUInt32BE(12), 7, "tkhd v0 保留轨道 ID");
  assert.equal(tkhd.readUInt32BE(16), 0, "tkhd v0 保留 reserved 字段");
...

{
  ...
  const tkhd1 = box("tkhd", (() => { const payload = Buffer.alloc(96); payload[0] = 1; payload.writeBigUInt64BE(123n, 12); payload.writeUInt32BE(7, 20); payload.writeBigUInt64BE(10_000n, 28); return payload; })());
  ...
  const trakBox = boxes(moov).find(item => item.type === "trak").data;
  // tkhd v1: version+flags(4) + creation(8) + modification(8) + trackId(4) + reserved(4) → duration at 28.
  const tkhd = boxes(trakBox).find(item => item.type === "tkhd").data;
  assert.equal(Number(tkhd.readBigUInt64BE(28)), 12_500, "tkhd v1 时长使用影片时间刻度");
  assert.equal(Number(tkhd.readBigUInt64BE(12)), 123, "tkhd v1 保留修改时间");
  assert.equal(tkhd.readUInt32BE(20), 7, "tkhd v1 保留轨道 ID");
  assert.equal(tkhd.readUInt32BE(24), 0, "tkhd v1 保留 reserved 字段");
...
if (process.env.LMD_SKIP_FFMPEG_TEST !== "1") {
  ...
  try {
    ...
    for (const trak of boxes(moov).filter(item => item.type === "trak")) {
      const tkhd = boxes(trak.data).find(item => item.type === "tkhd").data;
      const trackDuration = tkhd[0] === 1 ? Number(tkhd.readBigUInt64BE(28)) : tkhd.readUInt32BE(20);
      assert.equal(trackDuration, Math.round(123.456 * mvhd.readUInt32BE(version === 1 ? 20 : 12)), "真实 FFmpeg tkhd 时长应匹配影片时长");
      assert.equal(tkhd.readUInt32BE(tkhd[0] === 1 ? 24 : 16), 0, "真实 tkhd reserved 字段不能被时长覆盖");
    }
