...
async function openFirstMedia(session) {
  ...
  await session.waitFor("document.querySelector('.mcard-hit, .media-card .card-hit-area')", { label: "目录卡片", timeout: 25000 });
  for (let depth = 0; depth < 6; depth++) {
    if (await session.evaluate("Boolean(document.querySelector('.mcard-hit[aria-label^=\"播放 \"], .card-hit-area[aria-label^=\"播放 \"]'))")) {
      await session.evaluate("document.querySelector('.mcard-hit[aria-label^=\"播放 \"], .card-hit-area[aria-label^=\"播放 \"]').click()");
      return depth;
    }
    if (!await session.evaluate("Boolean(document.querySelector('.mcard-hit[aria-label^=\"打开文件夹 \"], .card-hit-area[aria-label^=\"打开文件夹 \"]'))")) throw new Error("目录里没有可进入的卡片");
    await session.evaluate("document.querySelector('.mcard-hit[aria-label^=\"打开文件夹 \"], .card-hit-area[aria-label^=\"打开文件夹 \"]').click()");
    await delay(900);
  }
...
async function seekWithBar(session, value, dispatchInput = true) {
  const result = await session.evaluate(`(() => {
    ...
    if (!range) return { ok: false, reason: 'progress input missing' };
    ...
    const video = document.querySelector('.lmd-stage video');
    return { ok: true, wrote: range.value, target: Number('${value}'), max: range.max, videoTime: video?.currentTime, paused: video?.paused };
  })()`);
  if (!result?.ok) throw new Error(`进度条定位失败: ${JSON.stringify(result)}`);
  return result;
}

// A released session must stop answering; a lingering lease from another run is
// irrelevant to this assertion.
// Waits until every session this run created is gone. A cancelled unmount
// DELETE leaves the session without heartbeats, so the lease must reclaim it.
async function waitForRelease(ids, timeout) {
  const deadline = Date.now() + timeout;
  let states = [];
  while (Date.now() < deadline) {
    states = [];
    for (const id of ids) states.push({ id: id.slice(0, 8), state: await releasedSessionState(id), descriptor: await sessionDescriptor(id) });
    if (states.every(item => item.state !== "alive")) return states;
    await delay(2000);
  }
  return states;
}

// Age and idle time tell an orphaned session apart from one still heartbeated.
async function sessionDescriptor(sessionId) {
  const response = await fetch(`${BASE}/api/settings/video-playback`, { cache: "no-store" }).catch(() => null);
  if (!response?.ok) return null;
  const body = await response.json().catch(() => ({}));
  const entry = (body?.sessions || []).find(item => String(item.id).startsWith(sessionId.slice(0, 8)));
  return entry || null;
}
...
async function leavePlayer(session) {
  ...
  await session.waitFor("!document.querySelector('.lmd-stage')", { label: "播放页卸载", timeout: 10000 }).catch(() => {});
  return session.evaluate("!document.querySelector('.lmd-stage')");
}
...
async function main() {
  ...
  const sessionPosts = [];
  session.on("Network.requestWillBeSent", ({ request }) => { if (/\/playback-sessions$/.test(request.url)) sessionPosts.push(`${request.method} ${request.url.replace(BASE, "")} body=${String(request.postData || "").slice(0, 120)}`); });
  session.on("Network.loadingFailed", ({ requestId, errorText, canceled }) => { if (canceled) sessionPosts.push(`CANCELED ${requestId} ${errorText}`); });
  ...
  // while the player is mounted. A session left behind by an earlier run can
  // keep heartbeating past its lease, so pre-existing ids are excluded.
  const preexisting = new Set(await settingsSessions());
  console.log(`既有会话 ${preexisting.size} 个（不计入本轮断言）`);
  const seenSessions = new Set();
  const collectSessions = async () => { for (const id of await settingsSessions()) if (!preexisting.has(id)) seenSessions.add(id); };
  ...
  try {
    ...
    // Seek behaviour is only meaningful with an explicit "playing" intent: a
    // paused player is not supposed to resume itself, so make the intent
    // unambiguous instead of inferring it from a previous tap.
    const ensurePlaying = async () => {
      const state = await session.evaluate(`(async () => { const v = document.querySelector('.lmd-stage video');
        if (v.paused) { const button = document.querySelector('.lmd-controls button[aria-label="\u64ad\u653e"]'); if (button) button.click(); else await v.play().catch(() => {}); }
        await new Promise(r => setTimeout(r, 800));
        return { paused: document.querySelector('.lmd-stage video').paused }; })()`, { awaitPromise: true });
      return state.paused === false;
    };
    const playingBeforeSeek = await ensurePlaying();
    ...
    record("播放中定位不打断播放", playingBeforeSeek && warmStillPlaying === true, JSON.stringify({ playingBeforeSeek, warmStillPlaying }));
    ...
    // A real exit drops the video parameter so the player page unmounts; the
    // brand button alone stays on the player page in this app shell.
    const leftDesktop = await leavePlayer(session);
    record("离开播放页后播放器已卸载", leftDesktop === true, `unmounted=${leftDesktop}`);
    await delay(2500);
    const afterLeave = await health(session);
    // A session whose unmount DELETE the browser cancels stops being
    // heartbeated and expires with its lease, so wait the lease window out.
    const trackedStates = await waitForRelease(seenSessions, 60000);
    ...
    await leavePlayer(session);
    await delay(2500);
    const afterMobileLeave = await health(session);
    const mobileStates = await waitForRelease(seenSessions, 60000);
