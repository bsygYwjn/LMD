...
try {
  ...
  let errors = ""; server.stderr.on("data", b => errors += b); server.stdout.resume();
  ...
  console.log(JSON.stringify({ result: "passed", tested: ["Range", "dual copy", "shared pipeline", "seek PTS", "stale generation", "DTS audio-only", "full transcode", "release"], elapsedMs: Math.round(performance.now() - start), status }));
