...
await mkdir(path.join(library, 'Other'));
const other = { ...media, id: 'other-video', path: path.join(library, 'Other', 'other-01.mp4'), fileName: 'other-01.mp4' };
await writeFile(other.path, 'immutable test video');
await writeFile(path.join(data, 'state.json'), JSON.stringify({ version: 11, libraries: [{ id: 'lib', name: '测试视频库', path: library }], media: [media, other], jobs: [], settings: { autoScanEnabled: false, autoPrepareCompatibleCopies: false }, displayGroups: [] }));
...
try {
  ...
  const root = (await req('/api/labels/targets')).json.find(t => t.name === 'videos' && t.kind === 'folder');
  assert.equal(root.status, 'pending', 'a completed first work does not mark the root complete');
  assert.equal(root.parentId, null);
  assert.equal(root.counts.pending, 1);
  assert.equal(root.counts.completed, 1);
  const scoped = (await req('/api/labels/status?folder=Work')).json;
  assert.equal(scoped.completed, 1);
  assert.equal(scoped.pending, 0);
  const next = (await req('/api/labels/claim', { folder: 'videos' })).json;
  assert.equal(next.groups.length, 1, 'a repeated default claim continues into remaining descendants');
  assert.equal(next.groups[0].directory, 'Other');
  const nextResult = { batchId: next.batchId, groups: next.groups.map(g => ({ ...result.groups[0], id: g.id, version: g.version })) };
  assert.equal((await req('/api/labels/apply', nextResult)).json.success, 1);
  assert.equal((await req('/api/labels/status?folder=videos')).json.pending, 0);
  assert.equal((await req('/api/labels/claim', { folder: 'videos' })).json.groups.length, 0);
